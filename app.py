...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgres://jacobkreinik@localhost/jkreinikHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    priority = StringField('Enter the new priority of this list item', validators=[Required()])
    update = SubmitField('Update')
...
def update(item):
    ...
    if form.validate_on_submit():
        update_priority = form.priority.data
        current = TodoItem.query.filter_by(description = item).first()
        current.priority = update_priority
        db.session.commit()

        flash ('The priority of the item {} has been updated'.format(item))
        return redirect(url_for('all_lists'))
...
def delete(lst):
    ...
    flash('To do list {} has been removed'.format(current_lst.title))
    return redirect(url_for('all_lists'))
    # Replace with code
